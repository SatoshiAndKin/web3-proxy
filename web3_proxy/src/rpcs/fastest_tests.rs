...
impl Fleet {
    ...
    pub(super) async fn with_backups(
        ...
        Self::with_pool(synced, total, config, backups, None).await
    }

    pub(super) async fn with_secondary_pool(
        total: usize,
        select: fn(&mut App) -> &mut Arc<super::many::Web3Rpcs>,
    ) -> Self {
        Self::with_pool(1, total, AppConfig::default(), &[], Some(select)).await
    }
    ...
    async fn with_pool(
        ...
        config: AppConfig,
        backups: &[usize],
        secondary_pool: Option<fn(&mut App) -> &mut Arc<super::many::Web3Rpcs>>,
    ) -> Self {
        ...
        let secondary_nodes: Vec<_> = nodes.iter().skip(1).map(|node| node.rpc.clone()).collect();
        let app = Arc::get_mut(&mut nodes[0].app).unwrap();
        if let Some(select) = secondary_pool {
            let (pool, background, _) = super::many::Web3Rpcs::spawn(
                super::many::Web3RpcsSpawnConfig::new(1, None, 0, 0, 1_000_000),
                "secondary-pool".into(),
                app.frontend_tasks.clone(),
                app.frontend_shutdown.subscribe(),
                None,
                None,
            )
            .await
            ...
            *pool.by_name.write() = secondary_nodes
                .into_iter()
    ...
    pub(super) fn start(
        ...
        super::test_support::spawn(async move { pool.try_proxy_connection(&request).await })
    }
    ...
    pub(super) async fn frontend(&self) -> (u16, Frontend) {
...
async fn fastest_existing_websocket_uses_reloaded_count() {
    super::test_support::with_cleanup(async {
        ...
        for (configured, dispatched) in [(2, 2), (1, 1), (0, 3)] {
            ...
            let task = super::test_support::spawn(async move {
